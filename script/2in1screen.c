...
void rotate_screen(){
	...
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "Wacom HID 494A Finger touch", COOR[current_state]);
	system(command);
        sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "Wacom HID 494A Pen stylus", COOR[current_state]);
        system(command);
        sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "Wacom HID 494A Pen eraser", COOR[current_state]);
        system(command);
}
